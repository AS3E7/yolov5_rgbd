...
@author: HYD
"""
print('***************监仓前端算法版本v2.1.5 20210820***************\n')

#import _init_path
import os
import sys
thisfile = os.path.abspath(__file__)
filepath, filename = os.path.split(thisfile)
demo_path = filepath + '/demo'
sys.path.append(demo_path.replace('\\', '/'))
...
from util.Eval import PlotDeteResult
...
#            lgmsg.debug('    config info = {}'.format(ConfigInfo))
    ...
    if SelectUsingDataType == 1: # 输入数据模式1：点云 + 深度
        ...
        # 结合两类数据检测
        CombineTwoModeDataFlag = 2 # CombineTwoModeDataFlag=1, 通过depth 数据质量选择使用的数据类型
                                   # CombineTwoModeDataFlag=2, 通过 RGBD 模型检测
        if CombineTwoModeDataFlag == 1:
            # 判断使用数据类型
            ...
            if CurFrmSelectImageType == 'colormap':
                ...
                ImgDeteResult, AlarmResult = detect_person.DetectDepth(DepthData, Pts, PtsDeteBbox, DepthWidth, DepthHeight, WorldTime, PreDete, ConfigInfo, DebugFlag = DebugFlag)
        elif CombineTwoModeDataFlag == 2:
            RGBImageData = np.reshape(RGBData, (RGBHeight, RGBWidth,3)) # [540x960x3]
            ImgDeteResult, AlarmResult = detect_person.DetectRGBD(RGBImageData, RGBWidth, RGBHeight, DepthData, Pts, PtsDeteBbox, DepthWidth, DepthHeight, WorldTime, PreDete, ConfigInfo, DebugFlag = DebugFlag)

        
    # 是否保存结果
    if ConfigSaveDetectBboxImageFlag == 1:
        if ImgDeteResult['Data'].shape[0] == 6: # 两类数据
            ResultSaveFileName1 = os.path.join(SaveResultDir, 'image', str(time.time()) + '_1.png')
            PlotDeteResult(ImgDeteResult['Data'][:3,:,:], ImgDeteResult['Bbox'], ImgDeteResult['Label'], ImgDeteResult['Score'], ResultSaveFileName1)
            ResultSaveFileName2 = os.path.join(SaveResultDir, 'image', str(time.time()) + '_2.png')
            PlotDeteResult(ImgDeteResult['Data'][3:,:,:], ImgDeteResult['Bbox'], ImgDeteResult['Label'], ImgDeteResult['Score'], ResultSaveFileName2)
        else: # 单类数据
            ResultSaveFileName = os.path.join(SaveResultDir, 'image', str(time.time()) + '.png')
...
#        logging.debug('  SelectUsingDataType = {}'.format(SelectUsingDataType))
    ...
    if len(SaveFileName)>1 and ConfigSaveDetectBboxImageFlag==1:
        if ImgDeteResult['Data'].shape[0] == 6: # 两类数据, [424,512,6]
            ResultSaveFileName1 = os.path.join(SaveResultDir, 'image', SaveFileName + '_1.png')
            PlotDeteResult(ImgDeteResult['Data'][:3,:,:], ImgDeteResult['Bbox'], ImgDeteResult['Label'], ImgDeteResult['Score'], ResultSaveFileName1)
            ResultSaveFileName2 = os.path.join(SaveResultDir, 'image', SaveFileName + '_2.png')
            PlotDeteResult(ImgDeteResult['Data'][3:,:,:], ImgDeteResult['Bbox'], ImgDeteResult['Label'], ImgDeteResult['Score'], ResultSaveFileName2)
        else: # 单类数据, [424,512,3]
            ResultSaveFileName = os.path.join(SaveResultDir, 'image', SaveFileName + '.png')
            PlotDeteResult(ImgDeteResult['Data'], ImgDeteResult['Bbox'], ImgDeteResult['Label'], ImgDeteResult['Score'], ResultSaveFileName)
