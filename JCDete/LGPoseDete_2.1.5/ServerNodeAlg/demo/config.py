...
if cfg_case == 1:
    ...
    multi_sensor_params = {
              ...
              'alone_stay_sensor_group': [[6,7,8,9,10],[1,2,3,4,5]], # 单人留仓内外仓传感器分组序号： [默认值：None]
              
              'internal_supervisor_continuous_time': 0.1, # 内部监管持续多长时间，单位：min
...
#                         'detect_bbox_model_name': 'LG_Depth_20200731', # 选择目标检测模型名称，【NewLG_OneClass_20200612】
...
alarm_level_first = {
                    ...
                    # 单人留仓
                    'alone_continuous_frame_num': 5, # 单人留仓 连续多帧有效
                    'alone_continuous_time': 0, # 单人留仓 超时时长，单位：分钟
                    # 超高
                    'above_height_continuous_frame_num': 3, # 超高 连续多帧有效
                    'above_height_continuous_continuous_time': 0, # 超高 连续时间，单位：分钟
                    ...
                    # 未在指定时间休息
                    'wrong_time_nolie_frame_num': 15, # 未在指定时间休息 连续多帧有效
                    'wrong_time_nolie_continuous_time': 0.25, # 未在指定时间休息 连续时间，单位：分钟
                    # 非休息时间休息
                    'wrong_time_lie_frame_num': 15, # 非休息时间休息 连续多帧有效
                    'wrong_time_lie_continuous_time': 0.25, # 非休息时间休息 连续时间，单位：分钟
                    }
alarm_level_second = {
                    # 多个功能通用参数设置
                    ...
                    # 厕所区域异常
                    'toilet_continuous_time': 3, # 厕所区域 超时时长，单位：分钟
                    # 未在指定时间休息
                    'wrong_time_nolie_frame_num': 15, # 未在指定时间休息 连续多帧有效
                    'wrong_time_nolie_continuous_time': 1, # 未在指定时间休息 连续时间，单位：分钟
                    # 非休息时间休息
                    'wrong_time_lie_frame_num': 15, # 非休息时间休息 连续多帧有效
                    'wrong_time_lie_continuous_time': 1, # 非休息时间休息 连续时间，单位：分钟
                    }
alarm_level_third = {
                    # 多个功能通用参数设置
                    ...
                    # 未在指定时间休息
                    'wrong_time_nolie_frame_num': 15, # 未在指定时间休息 连续多帧有效
                    'wrong_time_nolie_continuous_time': 2, # 未在指定时间休息 连续时间，单位：分钟
                    # 非休息时间休息
                    'wrong_time_lie_frame_num': 15, # 非休息时间休息 连续多帧有效
                    'wrong_time_lie_continuous_time': 2, # 非休息时间休息 连续时间，单位：分钟
                    }
alarm_level = { # 告警等级总和
               '1': alarm_level_first, 
