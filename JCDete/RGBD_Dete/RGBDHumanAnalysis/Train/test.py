...
from utils.general import (
    ...
    xyxy2xywh, clip_coords, plot_images, xywh2xyxy, box_iou, output_to_target, ap_per_class, set_logging, compute_loss_multi_output)
...
def test(data,
    ...
    if not training:
        ...
        dataloader = create_dataloader(path, imgsz, batch_size, model.stride.max(), opt,
                                       hyp=hyp, augment=False, cache=False, pad=0.5, rect=True)[0]
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            if training:  # if model has loss hyperparameters
                if hyp['inputdata_channel_num']<=3:
                    ...
                    loss += compute_loss_multi_output([x.float() for x in train_out], targets, model)[1][:3]  # GIoU, obj, cls
        ...
        for si, pred in enumerate(output):
            if isinstance(targets, dict): # 3 类 label
                labels = targets['rgbd'][targets['rgbd'][:, 0] == si, 1:]
            else:
        ...
        if batch_i < 1:
            if img.shape[2]<=3:
                ...
                f = Path(save_dir) / ('test_batch%g_gt.jpg' % batch_i)  # filename
                if isinstance(targets, dict): # 3 类 label
                    plot_images(img[:,:3,:,:], targets['rgbd'], paths, str(f), names)  # ground truth
                else: # 1 类 label
                    plot_images(img[:,:3,:,:], targets, paths, str(f), names)  # ground truth
                f = Path(save_dir) / ('test_batch%g_pred.jpg' % batch_i)
                plot_images(img[:,:3,:,:], output_to_target(output, width, height), paths, str(f), names)  # predictions
...
if __name__ == '__main__':
    ...
    # test images
    test(opt.data, weights=opt.weights, batch_size=opt.batch_size, imgsz=opt.img_size, conf_thres=opt.conf_thres, iou_thres=opt.iou_thres
