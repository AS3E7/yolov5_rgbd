...
if __name__ == '__main__':
    ...
    SelectWeightNameModel = r'weights\lg_c3_rgbd_2branch_ch6_07032100.pt'
    ...
    if TestCase == 1: # 从图片开始测试
        ...
        ImageFolderName = r'data\test_data\test_ply'
        ResultFolderName = r'data\test_data\result'
        # TransMatrixFileName = r'data/rgb2depth_param.txt'
        TransMatrixFileName = r'data\RGB2DepthRotParams\ZT\15/rot_param.txt'
    ...
    if 0: # test RGB2Depth
        ...
        TestRGBImageFileName = r'D:\xiongbiao\Code\LGPoseDete\RGBDHumanAnalysis\Train\data\test_data\test_ply\15_Color-10001-2021-07-13-144603_00100.png'
        TestDepthImageFileName = r'D:\xiongbiao\Code\LGPoseDete\RGBDHumanAnalysis\Train\data\test_data\test_ply\15_Depth-10001-2021-07-13-144603_00100.png'
    
        TransMatrixFileName = r'D:\xiongbiao\Code\LGPoseDete\RGBDHumanAnalysis\Train\data\RGB2DepthRotParams\ZT\15/rot_param.txt'
    
        img_depth = cv2.imread(TestDepthImageFileName)
        img_rgb = cv2.imread(TestRGBImageFileName)
        dest_image_width = img_depth.shape[1]
        dest_image_height = img_depth.shape[0]
    
        # =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
        RGB2DepthTransformMatrix = RGB2Depth.ReadTransformFile(TransMatrixFileName)
        ...
        DestImage_trans = cv2.warpAffine(img_rgb,RGB2DepthHCV,(dest_image_width, dest_image_height)) # matrix=[2,3], DestImage=[424,512,3]
        
        cv2.imwrite('trans_rgb.png', DestImage_trans)
        
        # =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
        RGB2DepthTransformMatrix2 = RGB2Depth.ReadTransformFile(TransMatrixFileName)
        SrcImgWidth = img_rgb.shape[1]
        SrcImgHeight = img_rgb.shape[0]
        RGB2DepthTransformMatrixInv = np.linalg.inv(RGB2DepthTransformMatrix2)
        ...
        for i_h in range(dest_image_height):
            for i_w in range(dest_image_width):
                ...
                if NewPoints[1][0] < 0 or NewPoints[1][0] >= SrcImgWidth:
                    continue
                DestImage[i_h, i_w, 0] = img_rgb[int(NewPoints[0][0]), int(NewPoints[1][0]), 0]
                DestImage[i_h, i_w, 1] = img_rgb[int(NewPoints[0][0]), int(NewPoints[1][0]), 1]
                DestImage[i_h, i_w, 2] = img_rgb[int(NewPoints[0][0]), int(NewPoints[1][0]), 2]
        cv2.imwrite('trans_rgb_loop.png', DestImage)
