...
class DetectDatasetImages():
    ...
    def detect_images(image_dir, label_dir, model, txt_dir=None, save_dir=None, opt=None, trainval_type='train',dataset_dir=None):
        ...
        if (txt_dir == None) or (not os.path.exists(txt_dir)):
            ...
            for i_file in files_lines:    
                ...
                elif i_file.endswith('.jpg\n'):
                    if dataset_dir == None:
                        cur_i_file = i_file.replace('./', 'data/').replace('.jpg\n', '.jpg') # trans current file name
                    else:
                        cur_i_file_1 = i_file.replace('./coco/', '').replace('.jpg\n', '.jpg') # trans current file name
                        cur_i_file = os.path.join(dataset_dir, cur_i_file_1)
                    files.append(cur_i_file)
                else:
        ...
        with torch.no_grad():
            for i in files:
                ...
                # if not ('1001_Depth-2021-10-21-221129_10005.png' in i):
                #     continue
                # aaa = 0
                        ...
                        for *xyxy, conf, cls in reversed(bbox):
                            ...
                            if img0.shape[2] <= 3:
                                ...
                                if SaveTwoPlotResultFlag==True:
                                    cv2.rectangle(plot_img1, (int([*xyxy][0]),int([*xyxy][1])), (int([*xyxy][2]),int([*xyxy][3])), cur_color, 5)
    ...
    def calc_dataset_acc(image_dir, label_dir, save_dir=None, opt=None, trainval_type='train', txt_dir=None):
        ...
        yolov3_file = os.path.join(path_labels, '*.txt')
            ...
            if not txt_dir==None:
                ...
                for i_file in files_lines: 
                    if i_file.endswith('.png\n'):
                        ...
                        if not os.path.exists(os.path.join(label_dir, cur_i_file)):
                            continue
                        ...
                        cur_i_file = os.path.basename(i_file).replace('.jpg\n', '.txt') # trans current file name
                        if not os.path.exists(os.path.join(label_dir, cur_i_file)):
                            continue
                        coco_file_dirs.append(os.path.join(label_dir, cur_i_file)) # coco_file
                        cur_i_file_pred = cur_i_file.replace('.txt', '.jpg.txt')
                        yolov3_file_dirs.append(os.path.join(path_labels, cur_i_file_pred)) # yolov3_file
        ...
        for ii, seq_dets_fn in enumerate(coco_file_dirs):
            ...
            img_seq = seq.split('.')[0]
            print('calc_dataset_acc img_seq:',img_seq)
            
            # if '142_Depth2020-08-10-151256_00300' == img_seq:
            #     print('im1g_seq')
            ...
            cur_image_postfix = '.png' # 图片后缀
            if os.path.exists(yolov3+'/'+labels+'/'+train2014+'/'+img_seq + '.png.txt') or os.path.exists(yolov3+'/'+labels+'/'+train2014+'/'+img_seq + '.jpg.txt'):
                if os.path.exists(yolov3+'/'+labels+'/'+train2014+'/'+img_seq + '.png.txt'):
                    ft = open(os.path.join(yolov3, labels, train2014, img_seq + '.png.txt'), 'r')
                if os.path.exists(yolov3+'/'+labels+'/'+train2014+'/'+img_seq + '.jpg.txt'):
                    ft = open(os.path.join(yolov3, labels, train2014, img_seq + '.jpg.txt'), 'r')   
                    cur_image_postfix = '.jpg'
            else:
                count_fp = len(fp.readlines())
                if not os.path.exists(coco+'/'+images+'/'+train2014+'/'+img_seq + '.png'):
                    cur_image_postfix = '.jpg'
                img = cv2.imread(coco+'/'+images+'/'+train2014+'/'+img_seq + cur_image_postfix)
                font = cv2.FONT_HERSHEY_SIMPLEX
                cv2.putText(img, str(count_fp), (20, 40), font, 1, (0, 0, 255), 5)
                cv2.imwrite(output+'/'+img_seq + cur_image_postfix, img)
                
                if os.path.exists(coco+'/'+images+'/'+train2014+'/'+img_seq + '_RGB' + cur_image_postfix):
                    img = cv2.imread(coco+'/'+images+'/'+train2014+'/'+img_seq + '_RGB' + cur_image_postfix)
                    font = cv2.FONT_HERSHEY_SIMPLEX
                    cv2.putText(img, str(count_fp), (20, 40), font, 1, (0, 0, 255), 5)
                    cv2.imwrite(output+'/'+img_seq + '_RGB' + cur_image_postfix, img)
    ...
    def calc_dataset_acc_obj(image_dir, label_dir, save_dir=None, opt=None, trainval_type='train', txt_dir=None, cls_names=['person'], cls_trans=3):
        ...
        for ii, seq_dets_fn in enumerate(coco_file_dirs):
            ...
            # debug
            # if not seq == 'Depth-2020-10-26-220702_10010.txt':
            #     continue
            ...
            img_seq = seq.split('.')[0]
            print('calc_dataset_acc_obj img_seq:',img_seq)
            
            # if 'Depth-2020-10-22-164112_10004' == img_seq:
            #     print('img_seq')

            
            # read truth image
            cur_image_postfix = '.png' # 图片后缀
            if not os.path.exists(os.path.join(coco, images, train2014, img_seq+'.png')):
                cur_image_postfix = '.jpg'
            cur_image_truth_name = os.path.join(coco, images, train2014, img_seq+cur_image_postfix)
            ...
            cur_image_pred_label_name = os.path.join(yolov3, labels, train2014, img_seq+ cur_image_postfix + '.txt')
            ...
            curr_image_iou_match_res = np.zeros([len(label_name), 3]) # objects ious
            for i_keys in cur_image_truth_label_info.keys():
                ...
                if len(cur_label_truth_obj_info)>0:
                    if len(cur_label_pred_obj_info)>0:
                        ...
                        curr_image_iou_match_res[i_label_name] = curr_match_res
            ...
            for i_keys in cur_image_truth_label_info.keys():    # 多类检测结果，只要有一个出现错误则保存图片
                i_label_name = i_keys
                if curr_image_iou_match_res[i_label_name][1]>0 or curr_image_iou_match_res[i_label_name][2]>0:
...
if __name__ == "__main__":
    ...
    if TestCase == 1:
        ...
        if TestCase == 3:
            ...
            TestDatasetFolderName = r'I:\MDT\project\RGBD\JCDete\RGBD_Dete\Dataset\RGBD\data_class3_lg\coco' # RGBD数据
        
            # zt_model, [lie, sit, stand], 
            TestModelWeightsName = [
                                    r'weights/lg_c3_depth_08260400.pt', # Depth
                                    r'weights/lg_c3_rgb_08260400.pt',
                                    r'weights/lg_c3_rgbd_2branch_ch6_07032100.pt',
                                    # r'', 
                                    # r'',
                                    r'weights/lg_c3_rgbd_l_ch6__07051800',
                                    ]
            SelectTestModelNameIdx = [1,1,1,0,0,0]

        


        TestDatasetClassNum = 3 # 3Class/1Class
        TestSaveResultFolderName = r'output\all_model_detect_result' # save folder name
        for i_model, CurTestModelName in enumerate(TestModelName):
            ...
            if CurTestModelName == 'Depth':
                CurModelProjectFolderName = r'I:\MDT\project\RGBD\JCDete\RGBD_Dete\RGBDHumanAnalysis\Test'
                DataImageType = 'depth'
            ...
            SelectWeightNameModel = os.path.join(CurModelProjectFolderName, CurModelWeightsName)
