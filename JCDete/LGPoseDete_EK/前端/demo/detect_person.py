...
def DetectDepth(DepthData, Pts, PtsDeteBbox, DepthWidth, DepthHeight, WorldTime, PreDete, ConfigInfo, DebugFlag = 0):
    """
    功能：使用深度数据检测目标
    输入：
...
#    print('ProcessDepth size = ', ProcessDepth.shape)
    ...
    # 功能检测
    AlarmResult = AlarmDete(PredBbox, PredLabels, PredScores, ProcessPts, DepthWidth, DepthHeight, DepthToCloudPointIdx, WorldTime, ConfigInfo, PreDete, DebugFlag, SrcPts = PtsDeteBbox)

    return CurImgDeteResult, AlarmResult
    
def DetectRGBDepth(RGBImageData, RGBWidth, RGBHeight, DepthData, Pts, PtsDeteBbox, DepthWidth, DepthHeight, WorldTime, PreDete, ConfigInfo, DebugFlag = 0):
    """
