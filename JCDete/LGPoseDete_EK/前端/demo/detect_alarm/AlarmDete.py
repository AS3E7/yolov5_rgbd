...
AboveHeightSensorOverlapDist = 0.2 # 传感器交接处重叠范围
...
AboveHeightSecondMethodFlag = 2 # 方法1：使用中间设置的一个高度计算是否超高，判断是否有两个高度超高
...
def AlarmDete(ImgDeteBbox, ImgDeteLabel, ImgDeteScore, Pts, ImgWidth, ImgHeight, DepToCptIdx, WorldTime, ConfigInfo, PreDete, DebugFlag = 0, SrcPts = None):
    """
...
#                                    if (ImgDeteLabel[nump] != CurModelLyingLabel) and (PredInfo[nump][2] < DeteMinHeight): # 如果在指定区域非躺下,则告警， 未增加‘一定高度有数据点’： PredAreaPtsLoc[i_area, 0] == 1
                    ...
                    PredBedPos_Step1 = AlarmDete_Pts(PredBedPos_Cur, DeteArea, PredAreaPtsLoc, AlarmFrameNumThod, DeteContinuousTime, AlarmValidFlag, MinFrameNum = -1, MaxFrameNum = 2)
                    ...
                    PredBedPos = AlarmDete_Pts(PredBedPos_Cur, DeteArea, PredAreaPtsLoc, AlarmFrameNumThod, DeteContinuousTime, AlarmValidFlag, MinFrameNum = -1, MaxFrameNum = 2)
            else:
            ...
            if CurSensorId in CurALARM_INFO['DeteSensor']: # 当前传感器时候检测该功能
                ...
                DeteMinHeight = DeteBbox_BedHeight + 0.2 # 龙岗厕所区域设置高度：0.9， 为了排除桶的干扰，DeteBbox_BedHeight + 0.5
                NearMintuePeriodTime = AlarmValidNearMintueTime # 时间段设置前后 10 分钟缓冲时间
...
#                                        and ImgDeteLabel[nump] != CurModelLyingLabel and PredAreaPtsLoc[i_area, 0] == 1: # 目标在区域内+目标不是lying+目标一定高度有数据点
                        ...
                        # 根据多帧结果判断是否告警
                        AlarmValidFlag = True
                        PredBedPos = AlarmDete_Pts(PredBedPos, DeteArea, PredAreaPtsLoc, AlarmFrameNumThod, DeteContinuousTime, AlarmValidFlag, MinFrameNum = -1, MaxFrameNum = AlarmFrameNumThod+5)
        ...
        if CurSensorId in CurALARM_INFO['DeteSensor']: # 当前传感器时候检测该功能
            ...
            if ConfigInfoUpdate == True: # 如果文件有更新，则重置告警结果和告警起始时间
                ...
                if InPeriodTimeFlag == 1:
                    ...
                    # 根据多帧结果判断是否告警
                    PredTOILETPos,  PredTOILET_STARTTIME = AlarmDete_SequenceFrame(PredTOILETPos, AlarmFrameNumThod, DeteContinuousTime, WorldTime, STARTTIME = PredTOILET_STARTTIME, MinFrameNum = -1, MaxFrameNum = AlarmFrameNumThod+5)
...
#                print('  PredWINDOWPos re-setting.')
                ...
                if InPeriodTimeFlag == 1:
                    ...
                    # 根据多帧结果判断是否告警
                    PredWINDOWPos,  PredWINDOW_STARTTIME = AlarmDete_SequenceFrame(PredWINDOWPos, AlarmFrameNumThod, DeteContinuousTime, WorldTime, STARTTIME = PredWINDOW_STARTTIME, MinFrameNum = -1, MaxFrameNum = AlarmFrameNumThod+5)
                else: # 不在检测时间段内
                    # 根据目标框结果检测人员信息
                    PredWINDOWPos = AlarmDete_Loc(PredWINDOWPos, PredInfo, ImgDeteLabel, ImgDeteScore, DeteArea, DeteMinHeight, DeteMaxHeight)
                    # 设置未检测到人员
...
#                print('src PredAreaPtsLoc = {}'.format(PredAreaPtsLoc))
                ...
                if DeteAreaSecond is not None:
                    ...
                    if AboveHeightSecondMethodFlag == 1: # 方法1：使用中间设置的一个高度计算是否超高，判断是否有两个高度超高
                        ...
                        AboveHeightSecondMethod2_MinHeight = 1.75 # 人员最低高度
                        PredAreaPtsLocSecond = np.empty([0, 4]) # [目标有效标记, 位置x, 位置y, 位置z]
                        AllDeteValidAreaSecond = np.zeros([6,DeteAreaSecond.shape[0]])
                        # 遍历特殊超高目标区域
                        for i_area in range(DeteAreaSecond.shape[0]):
...
#                                        ConnectSampPts = NewPtsBGValid[:,ConnectSampPtsIdx] # [3 x N]
                                        ...
                                        # calculate connected
                                        CurPredAreaPtsLocConnectFlag = connected(ConnectSampPts, ConnectSampCenter, ConnectSampWinSizeX, ConnectSampWinSizeZ, GridSize2d=0.05)
    ...
    if CurAlarmTypeId in ConfigInfo['ALARM_INFO'].keys():
        ...
        if CurSensorId in CurALARM_INFO['DeteSensor']: # 当前传感器时候检测该功能
            ...
            if InPeriodTimeFlag == 1:
                ...
                # 根据多帧结果判断是否告警
                AlarmValidFlag = True
                PredHAULWINDOW = AlarmDete_Pts(PredHAULWINDOW, DeteArea, PredAreaPtsLoc, AlarmFrameNumThod, DeteContinuousTime, AlarmValidFlag, MinFrameNum = -1, MaxFrameNum = AlarmFrameNumThod+5)
            else: # 不在检测时间段内
                # 根据多帧结果判断是否告警
                PredAreaPtsLoc = np.zeros([DeteArea.shape[0], 4]) # [目标有效标记, 位置x, 位置y, 位置z]
                AlarmValidFlag = False
    ...
    if CurAlarmTypeId in ConfigInfo['ALARM_INFO'].keys():
        ...
        if CurSensorId in CurALARM_INFO['DeteSensor']: # 当前传感器时候检测该功能
            ...
            if InPeriodTimeFlag == 1:
                ...
                # 根据多帧结果判断是否告警
                AlarmValidFlag = True
                PredBUILDLADDER = AlarmDete_Pts(PredBUILDLADDER, DeteArea, PredAreaPtsLoc, AlarmFrameNumThod, DeteContinuousTime, AlarmValidFlag, MinFrameNum = -1, MaxFrameNum = AlarmFrameNumThod+5)
            else: # 不在检测时间段内
                # 根据多帧结果判断是否告警
                PredAreaPtsLoc = np.zeros([DeteArea.shape[0], 4]) # [目标有效标记, 位置x, 位置y, 位置z]
                AlarmValidFlag = False
    ...
    if CurAlarmTypeId in ConfigInfo['ALARM_INFO'].keys():
        ...
        if CurSensorId in CurALARM_INFO['DeteSensor']: # 当前传感器时候检测该功能
            ...
            if InPeriodTimeFlag == 1:
                ...
                # 根据多帧结果判断是否告警
                AlarmValidFlag = True
                PredSTANDQUILT = AlarmDete_Pts(PredSTANDQUILT, DeteArea, PredAreaPtsLoc, AlarmFrameNumThod, DeteContinuousTime, AlarmValidFlag, MinFrameNum = -1, MaxFrameNum = AlarmFrameNumThod+5)
            else: # 不在检测时间段内
                # 根据多帧结果判断是否告警
                PredAreaPtsLoc = np.zeros([DeteArea.shape[0], 4]) # [目标有效标记, 位置x, 位置y, 位置z]
                AlarmValidFlag = False
