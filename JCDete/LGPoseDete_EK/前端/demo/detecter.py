...
#            lgmsg.debug('    config info = {}'.format(ConfigInfo))
    ...
    if BGInfo.shape[1] > 0: # 有背景信息
        ...
        if ReadDeteBboxBGInfoFlag == True: # 目标检测是否读取使用信息
            PtsDeteBbox = CalImageSubBG(Pts, BGInfo, CalMethod = 1, DepthWidth = DepthWidth, DepthHeight = DepthWidth)
            Pts = PtsDeteBbox
        else: 
            PtsDeteBbox = Pts
            Pts = CalImageSubBG(Pts, BGInfo, CalMethod = 1, DepthWidth = DepthWidth, DepthHeight = DepthWidth)
        # Depth
