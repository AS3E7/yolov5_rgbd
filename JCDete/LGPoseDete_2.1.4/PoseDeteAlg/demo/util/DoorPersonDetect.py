...
if sys.platform == "win32": # windows
    # 聚类文件地址
    CurLibMeanShiftDir = os.path.join(os.path.dirname(ThisFilePath), 'meanshift', 'LibMeanShift.dll')
    LibMS = ctypes.CDLL(CurLibMeanShiftDir) 
else: # linux
    # 聚类文件地址
    CurLibMeanShiftDir = os.path.join(os.path.dirname(ThisFilePath), 'meanshift', 'MeanShift.so')
    LibMS = ctypes.CDLL(CurLibMeanShiftDir) 
# Cluster Function
MeanShiftWeightedCluster2D = LibMS.MeanShiftWeightedCluster2D
...
def DoorAreaCluster(NewPts, dist_threshold=0.3, MinPeopleHeight=0.6, MinCluPtNum=250):
    ...
    CluTbl0.nCluNum = nMaxCluNum
    res = MeanShiftWeightedCluster2D(
