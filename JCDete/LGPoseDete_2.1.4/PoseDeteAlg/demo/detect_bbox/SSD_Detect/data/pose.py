"""POSE Dataset Classes

Original author: Francisco Massa
https://github.com/fmassa/vision/blob/pose_dataset/torchvision/datasets/pose.py
...
class AnnotationTransform(object):
    """Transforms a POSE annotation into a Tensor of bbox coords and label index
    Initilized with a dictionary lookup of classnames to indexes

    Arguments:
        class_to_ind (dict, optional): dictionary lookup of classnames -> indexes
            (default: alphabetic indexing of POSE's 20 classes)
    ...
    def __init__(self, class_to_ind=None, keep_difficult=False):
        self.class_to_ind = class_to_ind or dict(
            zip(POSE_CLASSES, range(len(POSE_CLASSES))))
    ...
    def __call__(self, target, width, height):
        ...
        for obj in target.iter('object'):
            ...
            if name == 'head': # exclude label == 'head', 20181022
                continue
...
class POSEDetection(data.Dataset):
    """POSE Detection Dataset Object

    input is image, target is annotation

    Arguments:
        root (string): filepath to POSEdevkit folder.
